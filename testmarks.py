...
# 27-Jun-2015 ShaneG
#
# Generates a file to make marks at known localations in a random order.
#----------------------------------------------------------------------------
...
USAGE = """
Usage:
       %s [--width width] [--height height] [--xstep xstep] [--ystep ystep] [--safe safe_height] filename
"""
...
# First probe - use pos, safe
GCODE_FIRST = """
G0 X%0.4f Y%0.4f
G0 Z%0.4f(Quick move to probe clearance height)
"""

# Single probe step - use x, y, feedrate, safe
GCODE_STEP = """
G0 X%0.4f Y%0.4f
G0 Z0 F%0.4f
G0 Z%0.4f
...
if __name__ == "__main__":
  ...
  for y in range(options.ystep + 1):
    if (y % 2) == 0:
      ...
      for x in range(options.xstep + 1):
        points.append((x * dx / 25.4, y * dy / 25.4))
    else:
      # Odd lines
  ...
  # Do the first move
  output.write(GCODE_FIRST.strip() % (points[0][0], points[0][1], options.safe / 25.4) + "\n")
  # Do the individual probes
  for p in points:
    output.write(GCODE_STEP.strip() % (p[0], p[1], options.feedrate / 25.4, options.safe / 25.4) + "\n")
  # Generate the suffix
